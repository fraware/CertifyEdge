...
use crate::ast::{
    AtomicPredicate, ComparisonOperator, STLFormula, STLSpecification, TimeInterval, TimeUnit,
};
...
impl SMTTranslator {
    ...
    pub async fn translate(&mut self, spec: &STLSpecification) -> SmtOperationResult<SMTOutput> {
        ...
        let solvers_ok =
            z3_output.result != SolverVerdict::Error && cvc5_output.result != SolverVerdict::Error;
    ...
    fn translate_formula_to_smt(&self, formula: &STLFormula) -> String {
        match formula {
            ...
            STLFormula::Always(interval, formula) => {
                self.translate_temporal_op("always", interval, formula)
            }
            STLFormula::Eventually(interval, formula) => {
                self.translate_temporal_op("eventually", interval, formula)
            }
    ...
    async fn execute_with_z3(&self, script: &str) -> SmtOperationResult<SolverOutput> {
        ...
        let z3_path = self
            .config
            .get_z3_path()
            .map_err(|e| SMTError::SolverNotFound {
                solver: format!("z3 ({e})"),
            })?;

        // Create temporary file
        let mut temp_file = NamedTempFile::new().map_err(|e| SMTError::SolverExecutionFailed {
            output: e.to_string(),
        })?;

        temp_file
            .write_all(script.as_bytes())
            .map_err(|e| SMTError::SolverExecutionFailed {
                output: e.to_string(),
            })?;
...
mod tests {
    use super::*;
    use crate::ast::{AtomicPredicate, ComparisonOperator, STLFormula, STLSpecification};
    ...
    fn test_translate_comparison_operator() {
        let translator = SMTTranslator::new(&CompilerConfig::default());
        assert_eq!(
            translator.translate_comparison_operator(&ComparisonOperator::GreaterThan),
            ">"
        );
        assert_eq!(
            translator.translate_comparison_operator(&ComparisonOperator::LessThan),
            "<"
        );
    }
    ...
    fn test_parse_smt_result() {
        ...
        assert!(matches!(
            translator.parse_smt_result("sat"),
            SolverVerdict::Sat
        ));
        assert!(matches!(
            translator.parse_smt_result("unsat"),
            SolverVerdict::Unsat
        ));
        assert!(matches!(
            translator.parse_smt_result("unknown"),
            SolverVerdict::Unknown
        ));
        assert!(matches!(
            translator.parse_smt_result("error"),
            SolverVerdict::Error
        ));
    }
}
