...
impl Lean4Translator {
    ...
    fn translate_formula(&self, formula: &STLFormula, name: &str) -> String {
        match formula {
            ...
            STLFormula::Always(interval, formula) => {
                self.translate_temporal_op("□", interval, formula, name)
            }
            STLFormula::Eventually(interval, formula) => {
                self.translate_temporal_op("◇", interval, formula, name)
            }
            STLFormula::Until(interval, left, right) => {
                self.translate_until(interval, left, right, name)
            }
    ...
    async fn compile_and_validate(&self, code: &str) -> Lean4Result<Lean4Metadata> {
        let lean4_path =
            self.config
                .get_lean4_path()
                .map_err(|e| Lean4Error::CompilationFailed {
                    output: format!("lean path: {e}"),
                })?;

        // Create temporary file
        let mut temp_file = NamedTempFile::new().map_err(|e| Lean4Error::CompilationFailed {
            output: e.to_string(),
        })?;
